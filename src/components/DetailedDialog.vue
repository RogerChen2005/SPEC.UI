...
const currentViewingPage = computed<BaseImage>(() => props.editable ? uploadedPages.value[props.pageIndex] : generatedPages.value[props.pageIndex]);
...
const designSpecsKeys = computed<SpecType[]>(() => Object.keys(designSpecs.value) as SpecType[]);
</script>
          ...
          <v-window v-model="activeTab">
            <v-window-item value="layers">
              <div class="d-flex align-center justify-between mb-2">
                ...
                <v-spacer></v-spacer>
                <v-btn variant="tonal" @click="pageCompositionReference = props.pageIndex" v-if="editable">
              ...
              <LayerSelect :editable="editable" :current-tab="pageIndex"></LayerSelect>
            </v-window-item>
            ...
            <v-window-item value="ui">
              ...
              <template v-for="key in designSpecsKeys" :key="key">
                <div class="d-flex align-center justify-between mb-2">
                <h2 class="font-weight-bold text-h6">{{ designSpecs[key].label }}</h2>
                <v-spacer></v-spacer>
                <v-btn variant="tonal" @click="designSpecs[key].value = props.pageIndex" v-if="editable">
