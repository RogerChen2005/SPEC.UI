<template>
    <v-text-field density="compact" variant="solo-filled" flat hide-details prepend-inner-icon="mdi-magnify"
        placeholder="Search for..." class="my-2" v-model="keyword"></v-text-field>

    <v-select v-model="currentTab" :items="pages" item-title="name" item-value="id"
        variant="underlined" flat hide-details class="ma-2"></v-select>
    ...
    <v-window v-model="currentTab">
        <v-window-item v-for="(page, index) in pages" :key="index" :value="page.id">
            <v-list density="compact" nav>
                            ...
                            <template v-slot:activator="{ props }">
                                <v-list-item v-bind="props">
                                    <template v-slot:prepend>
                                        <v-checkbox-btn v-model="region.selected" @click.stop></v-checkbox-btn>
                                    </template>
                                    <template #title>
                ...
                <v-list-item v-else>
                    <v-list-item-subtitle v-if="!page.analysisComplete" class="text-center">Analyzing</v-list-item-subtitle>
                    <v-list-item-subtitle v-else class="text-center">No SPEC data</v-list-item-subtitle>
                </v-list-item>
            </v-list>
        </v-window-item>
    </v-window>
